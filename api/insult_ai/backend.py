...
Extracted from runner.py. Owns the two backend types (claude / codex),
their construction logic, and the cache that keeps the MCP subprocess
pool alive across turns. Runner imports ``_get_backend`` and
``normalize_backend_name``; nothing else needs to reach in here.
...
subscription auth. When an OAuth token is present we drop any ambient key
so the Max subscription wins. No-op in the container (the entrypoint env
carries no ANTHROPIC_API_KEY).

Cache rationale: constructing a ``ClaudeCodeBackend`` / ``CodexBackend``
spawns SDK clients and an MCP subprocess pool — ~1-3s overhead.
``_get_backend`` returns a process-wide singleton keyed by backend name
so the second chat turn reuses the same backend instance without
re-spawning ``npx @brightdata/mcp``.
"""
